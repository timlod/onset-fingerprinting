...
class RecAnalysis:
    ...
    def __init__(
        ...
        # TODO: think about a useful threshold for a better minimum minimum to
        # not allow to normalize to super quiet sounds in between sessions
...
class AnalysisOnDemand(RecAnalysis):
    ...
    def __init__(
        ...
        # Make sure there's just #.wav files in there, otherwise this fails
        wavs = sorted(config.WRITE_DIR.glob("*.wav"))
        if len(wavs) == 0:
            self.save_i = 0
        else:
            self.save_i = int(wavs[-1].stem) + 1
